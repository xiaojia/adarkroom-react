...
import { Outside as OutsideModule } from "../outside";
import { Room as RoomModule } from "../room";
...
export const Outside = [
	...
	title: _('A Ruined Trap'),
		isAvailable: function() {
			return Engine.activeModuleId === 'outside' && $SM.get('game.buildings["trap"]', true) > 0;
		},
		scenes: {
			'start': {
				text: [
				...
				onLoad: function() {
					...
					OutsideModule.updateVillage();
					OutsideModule.updateTrapButton();
				},
	...
	},
	{
		title: _('Fire'),
		isAvailable: function() {
			return Engine.activeModuleId === 'outside' && $SM.get('game.buildings["hut"]', true) > 0 && $SM.get('game.population', true) > 5;
		},
		scenes: {
			'start': {
				text: [
				...
				onLoad: function() {
					...
					var numKilled = 4;
					OutsideModule.killVillagers(numKilled);
	...
	{ /* Sickness */
		title: _('Sickness'),
		isAvailable: function() {
			return Engine.activeModuleId === 'outside' && $SM.get('game.population', true) > 10 && $SM.get('game.population', true) < 50 && $SM.get('stores.medicine', true) > 0;
		},
		scenes: {
			'start': {
				text: [
				...
				notification: _('a sickness is spreading through the village.'),
				blink: true,
				buttons: {
					'heal': {
						text: _('1 medicine'),
	...
	{ /* Plague */
		title: _('Plague'),
		isAvailable: function() {
			return Engine.activeModuleId === 'outside' && $SM.get('game.population', true) > 50 && $SM.get('stores.medicine', true) > 0;
		},
		scenes: {
			'start': {
				text: [
				...
				notification: _('a terrible plague is fast spreading through the village.'),
				blink: true,
				buttons: {
	...
	{ /* Beast attack */
		...
		isAvailable: function() {
			return Engine.activeModuleId === 'outside' && $SM.get('game.population', true) > 0;
		},
		scenes: {
			'start': {
				text: [
				...
				notification: _('a pack of snarling beasts pours out of the trees.'),
				onLoad: function() {
	...
	{ /* Soldier attack */
		...
		isAvailable: function() {
			return Engine.activeModuleId === 'outside' && $SM.get('game.population', true) > 0 && $SM.get('game.cityCleared');
		},
		scenes: {
			'start': {
				text: [
				...
				notification: _('a gunshot rings through the trees.'),
				onLoad: function() {
