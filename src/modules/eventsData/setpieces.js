...
export const Setpieces = {
		...
		scenes: {
			'start': {
				...
				onLoad: function() {
					...
					E().activeEvent().scenes[E().activeScene].loot = loot;
				},
				buttons: {
					'leave': {
						text: _('leave'),
	...
	"battlefield": { /* Discovering an old battlefield */
		...
		scenes: {
			'start': {
				...
				notification: _('a battle was fought here, long ago.'),
				onLoad: function() {
					World.markVisited(World.curPos[0], World.curPos[1]);
				},
				loot: {
	...
	"borehole": { /* Admiring a huge borehole */
		...
		scenes: {
			'start': {
				...
				notification: _('a huge hole is cut deep into the earth, evidence of the past harvest.'),
				onLoad: function() {
					World.markVisited(World.curPos[0], World.curPos[1]);
				},
				loot: {
	...
	"ship": { /* Finding a way off this rock */
		...
		scenes: {
			...
			notification: _('the familiar curves of a wanderer vessel rise up out of the dust and ash.'),
			buttons: {
					'leavel': {
						text: _('salvage'),
						nextScene: 'end'
